...
def load_user_data(_client):
    try:
        ...
        # Convert list of dicts to a dict keyed by username for easy lookup
        user_data = {user['username']: user for user in user_records}
        return user_data
    except Exception:
        return None
...
def add_user(client, new_user):
    try:
        ...
        worksheet.append_row([new_user, ""]) # Add user with empty webhook
        st.cache_data.clear()
        return True
    except Exception:
        return False
...
# --- 主程式 ---
st.set_page_config(page_title="國考訂正追蹤器 v10.0", layout="wide", page_icon="✍️")
...
if not st.session_state.logged_in_user:
    ...
    user_data = {"kudi68": {"username": "kudi68", "webhook_url": ""}} # Fallback
    if gs_client:
    ...
    with st.sidebar:
        ...
        # FIX: Added save button for webhook URL
        webhook_input = st.text_input("您的 Webhook 網址", value=st.session_state.webhook_url)
        if st.button("儲存"):
            if gs_client:
                if update_user_webhook(gs_client, st.session_state.logged_in_user, webhook_input):
